...
for i in range(NUM_ITERATIONS):
    ...
    for (future, key) in prev_futures:
        print(f"{BRIGHT_PINK}future key = {key} LOOP YYYYYYYYYYYYYYYYYYYYYYYYYYYYY{RESET}")
        while not future.done():
            print("\n")
            print (f"waiting for future XXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
            print("\n")
            # pass
        res = future.result()
        if res:
    ...
    prev_futures = []
    print(f"{BRIGHT_PINK}pref_fetch dic size is {len(prefetch_dict)}{RESET}")
