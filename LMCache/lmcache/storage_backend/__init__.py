...
from lmcache.storage_backend.local_backend_rai import LMCLocalBackend as LMCLocalBackend_rai
from lmcache.storage_backend.local_backend_rai import LMCLocalDiskBackend as LMCLocalDiskBackend_rai
...
def CreateStorageBackend(config: LMCacheEngineConfig,
    ...
    match config:
        case LMCacheEngineConfig(_, is_rai=True):
            ...
            match config.local_device:
                case "cpu" | "cuda":
                    ...
                    return LMCLocalBackend_rai(config, mpool_metadata, dst_device)
                case _:
                    logger.info(f"Initializing local-only (disk) backend at"
                                f" {config.local_device}")
                    return LMCLocalDiskBackend_rai(config, mpool_metadata,
                                               dst_device)
